...
本專案實例(偵錯過程本身是好素材):全新 broker 一起來就冒出別專案的
topics(`seckill.grabbed`…)。`lsof` 追查後真相:機器上有兩個跑了 12 天的
舊專案服務(搶票系統的 order/seckill)一直對 `localhost:9092` 重連,
而 lsof 裡那個「聽 9092 的 ssh」其實是 **Colima(Docker on macOS)的
port-forward 機制**——也就是我們容器自己的 9092 對外發布。所以流量真的
進了我們的 broker,`auto.create.topics.enable=true`(預設)讓它們自動建了 topic。
處置:關掉那兩個舊服務(Jeff 拍板)、broker 維持 9092;教訓固化進 compose:
- **雙 listener**:`INTERNAL://kafka:29092`(容器網路內)+
  `EXTERNAL://localhost:9092`(host 視角)——Docker 跑 Kafka 的標準姿勢。
- **關掉 `auto.create.topics.enable`**——資金系統的 topic 必須顯式建立,
  不接受「誰連上來就自動生 topic」;這次就是活生生的反例。
